...
optimizer = SAM(params_for_optimization, base_optimizer, lr=args.lr, weight_decay=args.weight_decay)
...
if not args.test:
    scheduler = get_cosine_schedule_with_warmup(
        optimizer.base_optimizer, 0, len(train_loader) * args.epochs, len(train_loader) * args.prev_epoch 
    )
    logging.info(
    ...
    best_epoch = 0
    for epoch in range(args.prev_epoch, args.epochs):
    ...
    model_filename = '/content/VL_Final/save_models/agqa/mist_SAM_agqa_v2/model-'+str(epoch)+'.pth'
    save_model_location = ' gs://mist_code/SAM_2'
    os.system('gsutil -m cp '+model_filename + save_model_location)
